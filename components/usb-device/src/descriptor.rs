...
impl DescriptorWriter<'_> {
    ...
    pub(crate) fn device(&mut self, config: &device::Config) -> Result<()> {
        self.write(
            ...
            &[
                0x00,
                0x02,                     // bcdUSB 2.00 — ВЕНДОРЕННЫЙ ФИКС: в апстриме 0x0210 (USB 2.1;
                // это корректный BCD, minor 0x10 = 1). Но 2.1 обязывает отдавать дескриптор BOS,
                // которого в usb-device 0.2.x нет (появился только в 0.3.x), поэтому Windows
                // отказывает в перечислении — «Сбой запроса дескриптора USB-устройства», Код 43.
                // Linux/macOS это поле прощают. Заявляем USB 2.0.
