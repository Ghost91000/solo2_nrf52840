...
impl<T: UsbPeripheral> Usbd<T> {
    ...
    fn read_control_setup(
        ...
        crate::trace::bump(&crate::trace::READ_SETUP_CALLS);
...

        {
            ...
            crate::trace::LAST_SETUP_TYPE.store(buf[0] as u32, Relaxed);
            crate::trace::LAST_SETUP_REQ.store(buf[1] as u32, Relaxed);
            crate::trace::LAST_SETUP_VALUE
                .store(((buf[3] as u32) << 8) | buf[2] as u32, Relaxed);
            crate::trace::LAST_SETUP_LEN.store(((buf[7] as u32) << 8) | buf[6] as u32, Relaxed);
        }
...
impl<T: UsbPeripheral> UsbBus for Usbd<T> {
    ...
    fn enable(&mut self) {
        interrupt::free(|cs| {
            ...
            errata::post_enable();
            // ВЕНДОРЕННЫЙ ФИКС (наш, в апстриме его нет).
            //
            // `EPSTATUS.EPIN[i]` = «EasyDMA-регистры захвачены»; по PS 6.35.13.28
            // снимается ТОЛЬКО записью 1, а драйвер чистит его лишь после
            // собственного успешного DMA. Наш загрузчик (Adafruit/nice!nano)
            // до прыжка в приложение сам эмулировал флешку через USBD и оставляет
            // этот бит установленным. Тогда `write()` для EP0 навсегда возвращает
            // `WouldBlock`, дескриптор не уходит хосту и Windows даёт «сбой запроса
            // дескриптора» / Code 43. На nRF52840-DK этого не происходит: там
            // приложение стартует после сброса чипа, а не после прыжка.
            // Маска: биты 0..8 = EPIN[0..8], биты 16..24 = EPOUT[0..8].
            const EP_MASK: u32 = 0x01FF_01FF;
            regs.epstatus.write(|w| unsafe { w.bits(EP_MASK) });
            regs.epdatastatus.write(|w| unsafe { w.bits(EP_MASK) });
    ...
    fn write(&self, ep_addr: EndpointAddress, buf: &[u8]) -> usb_device::Result<usize> {
        ...
        // Вендоренная трассировка: попытки ответить хосту через EP0.
        if i == 0 {
            crate::trace::bump(&crate::trace::WRITE_EP0_CALLS);
            crate::trace::WRITE_EP0_LAST_LEN
                .store(buf.len() as u32, core::sync::atomic::Ordering::Relaxed);
        }

        if usize::from(self.bufs.in_lens[i]) < buf.len() {
            if i == 0 {
                crate::trace::bump(&crate::trace::WRITE_EP0_OVERFLOW);
            }
            return Err(UsbError::BufferOverflow);
        }
        ...
        interrupt::free(|cs| {
            ...
            if busy_in_endpoints.get() & (1 << i) != 0 {
                ...
                if epdatastatus & (1 << i) != 0 {
                    ...
                    if i == 0 {
                        crate::trace::bump(&crate::trace::WRITE_EP0_BUSY_BIT);
                    }
            ...
            if regs.epstatus.read().bits() & (1 << i) != 0 {
                if i == 0 {
                    crate::trace::bump(&crate::trace::WRITE_EP0_EPSTATUS);
                }
                return Err(UsbError::WouldBlock);
            }
            ...
            if i == 0 {
                crate::trace::bump(&crate::trace::WRITE_EP0_OK);
            }
    ...
    fn poll(&self) -> PollResult {
        interrupt::free(|cs| {
            ...
            if regs.events_usbreset.read().events_usbreset().bit_is_set() {
                ...
                crate::trace::bump(&crate::trace::RESETS);
            ...
            if regs.events_sof.read().events_sof().bit_is_set() {
                ...
                if let TransferState::Started(counter) = state.in_transfer_state {
                    ...
                    if frame_counter.wrapping_sub(counter) >= 5 {
                        ...
                        crate::trace::bump(&crate::trace::EP0_TIMEOUT_STATUS);
                        regs.tasks_ep0status
                            .write(|w| w.tasks_ep0status().set_bit());
                        ...
                        // VENDORED FIX — upstream nrf-rs/nrf-usbd PR #29
                        // ("Fix enumeration on windows"), not in any release.
                        // The status stage above ends the transfer, so EP0 IN
                        // must be marked free again too. Without this the busy
                        // bit stays set forever, every later `write()` on EP0
                        // returns `WouldBlock`, and the host never receives the
                        // device descriptor (Windows: "сбой запроса дескриптора",
                        // Code 43).
                ...
                .bit_is_set()
            {
                ...
                if ep0_state.direction == UsbDirection::In {
                    ...
                    crate::trace::bump(&crate::trace::EP0_DATADONE_IN);
